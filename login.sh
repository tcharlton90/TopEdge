...
if [ -z $UNITY_USERNAME ] ; then
    echo "Please set the \$UNITY_USERNAME environment variable"
    exit 1
fi

if [ -z $UNITY_PASSWORD ] ; then
    echo "Please set the \$UNITY_PASSWORD environment variable"
    exit 1
fi

docker run -it --rm unityci/editor:ubuntu-2020.1.1f1-android-0.3.0 unity-editor -logFile /dev/stdout -batchmode -nographics -quit -username "$UNITY_USERNAME" -password "$UNITY_PASSWORD" | tee ./unity-output.log

cat ./unity-output.log | grep 'LICENSE SYSTEM .* Posting *' | sed 's/.*Posting *//' > "${activation_file}"
